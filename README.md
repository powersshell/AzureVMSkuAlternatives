...
### 2026-07-16
- **improvement:** Redesigned the **mode switcher** ("Find alternatives" / "Browse all VMs") into two prominent, high-contrast **cards** — each with an icon, bold title, and a one-line description — and added a live **count badge** on **Browse all VMs** showing how many VM sizes exist in the selected region.
- **fix:** **Browse all VMs** results are now fully scrollable on desktop — the results table scrolls within the page (with column headers pinned) so every row is reachable, and pagination stays visible at the bottom.
...
- **improvement:** Removed the redundant **CPU Vendor** filter from the source-SKU search area — vendor filtering now lives only on the **Alternatives** results, where it's most useful.
- **improvement:** The detailed comparison view now handles missing **ACU** values correctly (Azure only publishes ACU for certain — mostly older/mid-generation — VM series). Instead of a misleading zero-based delta, it shows "ACU: not published by Azure for at least one of these sizes" when either size lacks a published ACU.
