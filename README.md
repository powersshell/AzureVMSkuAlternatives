...
### 2026-08-17
- **fix:** **Searching a region by its portal display name — "Central US" instead of `centralus` — silently fell off the fast path, and from the MCP server it usually failed outright.** The SKU cache is partitioned on the exact ARM slug, and the region string was passed straight through to the partition-key filter with no normalization anywhere in the stack, so anything but the exact slug matched zero rows and fell back to a live Azure lookup. The damage differed by endpoint and none of it was obvious: `/skus` returned **HTTP 200 with an empty catalog** for `Central US` *and* for nonsense like `bogusregion`, so a typo looked like "this region has no VM sizes"; `/compare_vms` answered, but in **40–49s against 5.1s for the correct slug**, and returned a *different* set (**196 alternatives vs 192**) because the live path and the curated cache don't agree — the same region gave different answers depending on spelling. Worst hit was the **MCP server**, whose HTTP timeout is 60s: a 40–49s upstream call sat close enough to the ceiling that agent calls regularly failed with "Upstream request timed out". Region names are now normalized to the ARM slug at every entry point in both the API and the MCP server, so `"Central US"`, `"central us"`, `"Central-US"` and `"CENTRALUS"` all resolve identically. Reported from Microsoft Scout, which passed the display name through verbatim.
- **improvement:** Added a region-normalization regression test to the MCP guard. The guard shipped earlier today passes 28/28 — but only ever ran `eastus`, so a bug that depended entirely on *which* region you asked for was invisible to it. Also hardened the price-history query, which interpolated a caller-supplied SKU name into an OData filter unescaped.
