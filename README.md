...
### 2026-08-11
- **fix:** Constrained-vCPU **Dsv2** sizes (`Standard_DS11-1_v2`, `DS12-2_v2`, `DS13-4_v2`, …) weren't flagged as retiring — the retirement pattern didn't allow the `-N` suffix constrained variants carry. Seven sizes per region looked like clean alternatives despite retiring with the Dsv2 series on 2028-05-01. Now correctly badged.
- **fix:** The `Standard_B<n>ls` retirement entry was dead code, always shadowed by a broader B-series pattern. Reordered so narrower patterns match first.
- **improvement:** Documented the **VM SKU lifecycle runbook** (`docs/sku-lifecycle-runbook.md`) covering how a capacity-limited size transitions when it's later announced for retirement: **add the retirement entry, leave the growth-restriction entry in place**. The two signals are independent and both stay true — already the steady state for **102 sizes across 13 series**.
- **improvement:** Added `scripts/check_sku_lifecycle.py` to reconcile the hand-maintained retirement and capacity-limitation tables — Python↔PowerShell parity, inconsistent retirement coverage within a series, permanently shadowed patterns, dual-flag overlap, and Microsoft Learn link liveness. It found both fixes above.
- **feature:** The **Site Usage Analytics workbook** gained a **Where Users Find Answers** section showing how visitors navigate the site's two entry points — **Find Alternatives** and **Browse all VMs**. Framed as coverage rather than competition: surface coverage (who reaches each area, who uses both), daily unique users per area, engagement depth, a full breakdown of what users do in each area, the Browse → Find Alternatives handoff, movement between areas, and where the answer landed. Plus a **Site → MCP funnel** tile connecting site traffic to MCP server adoption.
- **improvement:** Every frontend telemetry event now records which area of the site the user was on when it fired, so shared features (price history, "where is this cheapest?", cross-region availability, exports) are attributable to where they were opened from. Still fully anonymous — no PII.
